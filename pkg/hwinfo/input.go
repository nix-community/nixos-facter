...
// pointerBaseClass maps udev's input classification onto the base class we report
// for input devices that libhd does not capture itself. Touchpads and touchscreens
// both live under the mouse hardware class, identified by base class, mirroring
// how libhd files touch devices under hw_mouse.
func pointerBaseClass(in *udev.Input) (BaseClass, bool) {
	switch {
	case in == nil:
		return 0, false
	case in.IsTouchpad:
		return BaseClassTouchpad, true
	case in.IsTouchscreen:
		return BaseClassTouchscreen, true
	default:
		return 0, false
	}
}

// captureInputDevices scans the input devices and identifies touchpads and touchscreens,
// which hwinfo does not capture, returning a slice of HardwareDevice structs or an error.
// It accepts a deviceIdx to ensure it continues on from the last device index generated by hwinfo.
func captureInputDevices(deviceIdx uint16) ([]HardwareDevice, error) {
	...
	for _, inputDevice := range inputDevices {
		...
		baseClass, ok := pointerBaseClass(udevData.Input)
		if !ok {
			// Currently, we are only interested in touchpads and touchscreens, as hwinfo does
			// not capture them. Eventually, we may take over more of the input processing that
			// hwinfo performs.
			continue
		}
		...
		hd := HardwareDevice{
			...
			BaseClass: NewBaseClassID(baseClass),
			Vendor: &ID{
